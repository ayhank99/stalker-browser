...
function requestHead(url, timeoutMs = 6000) {
  return new Promise((resolve, reject) => {
    let parsed;
    try { parsed = new URL(url); } catch (e) { return reject(e); }
    const protocol = parsed.protocol === 'https:' ? https : http;
    const req = protocol.request(parsed, {
      ...
      resp.resume();
      resolve({ statusCode: resp.statusCode, location: resp.headers && resp.headers.location });
    });
    req.setTimeout(timeoutMs, () => req.destroy(new Error('timeout')));
    req.on('error', reject);
    req.end();
...
async function fromInvidious(videoId) {
  for (const base of configuredInvidious()) {
    try {
      const r = await requestText(`${base}/api/v1/videos/${encodeURIComponent(videoId)}`, 8000);
      if (r.statusCode !== 200 || !r.body || r.body.trim().startsWith('<')) continue;
      let json;
      try { json = JSON.parse(r.body); } catch (_) { continue; }
      if (!json) continue;

      // Prefer HLS manifest - it's fully proxied through Invidious
      if (typeof json.hls === 'string' && json.hls) {
        return { url: json.hls, isM3U8: true, source: `invidious:${base}` };
      }

      // Fallback: find a proxied mp4 stream from adaptiveFormats
      const formats = Array.isArray(json.adaptiveFormats) ? json.adaptiveFormats : [];
      const mp4 = formats
        .filter(f => f && f.url && String(f.type || '').includes('video/mp4') && f.qualityLabel)
        .sort((a, b) => (Number(b.bitrate) || 0) - (Number(a.bitrate) || 0));
      if (mp4[0] && mp4[0].url) {
        return { url: mp4[0].url, isM3U8: false, isRedirect: false, source: `invidious:${base}` };
      }
    } catch (_) {}
  }
  return null;
...
// Invidious /latest_version proxies video through Invidious servers (local=true).
// This always avoids googlevideo.com on the caller side, making it cloud-friendly.
async function fromInvidiousLatestVersion(videoId) {
  // itag 18 = 360p combined H.264+AAC (available for almost all videos)
  // itag 22 = 720p combined (only for shorter videos)
  const itags = ['18', '22'];
  const instances = configuredInvidious();

  // Try instances in parallel batches of 3 for speed
  for (let i = 0; i < instances.length; i += 3) {
    const batch = instances.slice(i, i + 3);
    const results = await Promise.allSettled(
      batch.flatMap(base =>
        itags.map(async itag => {
          const url = `${base}/latest_version?id=${encodeURIComponent(videoId)}&itag=${itag}&local=true`;
          const r = await requestHead(url, 5000);
          if (r.statusCode >= 200 && r.statusCode < 300) {
            return { url, isM3U8: false, source: `invidious-lv:${base}:${itag}` };
          }
          // Some instances return 302 to googlevideo.com even with local=true — skip those
          if (r.statusCode >= 300 && r.statusCode < 400 && r.location && !isGoogleCdnUrl(r.location)) {
            return { url: new URL(r.location, base).toString(), isM3U8: false, source: `invidious-lv-redir:${base}:${itag}` };
          }
          return null;
        })
      )
...
function buildResolverList() {
  ...
  const customOrder = String(process.env.YT_RESOLVER_ORDER || '').split(',').map(v => v.trim()).filter(Boolean);
  if (customOrder.length) {
    return customOrder.filter(name => all[name]).map(name => [name, all[name]]);
  }

  // Cloud providers (Railway, Render, etc.) can't reach googlevideo.com directly.
  // Invidious /latest_version and Piped/Invidious APIs proxy through their own servers.
  if (isCloudEnvironment()) {
    return [
      ['invidious-lv', fromInvidiousLatestVersion],
      ['invidious', fromInvidious],
      ['piped', fromPiped],
      ['ytdl-core', fromYtdlCore],
      ['innertube', fromInnertube],
  ...
  for (const [name, resolver] of resolvers) {
    try {
      ...
      if (result && result.url) {
        if (skipGoogleCdn && isGoogleCdnUrl(result.url)) {
          attempts[attempts.length - 1].skipped = 'googlevideo';
          continue;
        }
        const value = { ...result, attempts };
        streamCache.set(cacheKey, { value, expiresAt: Date.now() + CACHE_MS });
        return value;
      }
    } catch (e) {
...
function invalidateStreamCache(videoId) {
  ...
  streamCache.delete(normalizedId);
  streamCache.delete(normalizedId + ':noCdn');
}

module.exports = { extractVideoId, buildProxyId, getStreamUrl, invalidateStreamCache, isGoogleCdnUrl };
