...
const { getStreamUrl, extractVideoId } = require('./yt-stream');
...
function proxyUrl(url, req, res, id, isM3U8, redirectCount) {
  ...
  const upstream = protocol.get(parsedUrl, {
    ...
    if (REDIRECT_STATUS.has(proxyRes.statusCode) && proxyRes.headers.location) {
      ...
      return proxyUrl(nextUrl, req, res, id, isM3U8, (redirectCount || 0) + 1);
    }
...
async function handle(req, res) {
  ...
  try {
    ...
    if (stream && stream.url && stream.isM3U8) {
      return proxyUrl(stream.url, req, res, id, true, 0);
    }

    if (stream && stream.url) {
      return proxyUrl(stream.url, req, res, id, false, 0);
    }

    return res.status(400).send('YouTube stream alinamadi. ' + (stream && stream.error || ''));
  } catch (e) {
